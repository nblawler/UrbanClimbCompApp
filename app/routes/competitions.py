...
from app.models import Account, Competition, Competitor, Section, SectionClimb, Score, LoginCode, DoublesTeam, DoublesInvite, Leaderboard
...
from app.helpers.leaderboard import build_leaderboard, get_competitor_position
...
def comp_competitor_sections(slug, competitor_id):
    ...
    # ── single indexed query instead of fetching all leaderboard rows ──
    position = get_competitor_position(target_id, current_comp.id)
...
def comp_competitor_stats(slug, competitor_id, mode="my"):
    ...
    # ── single indexed query instead of fetching all leaderboard rows ──
    position = get_competitor_position(competitor_id, current_comp.id)

    section_stats = []
    personal_heatmap_sections = []
    global_heatmap_sections = []

    MIN_TOPS_FOR_DIFFICULTY = 3
    HARD_AVG_ATTEMPTS = 6
    MED_AVG_ATTEMPTS = 3
