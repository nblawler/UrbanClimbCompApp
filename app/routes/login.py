...
from flask import (
    Blueprint, session, request, redirect, render_template, flash, abort
)
from app import db
from app.models import (
    Account, Competitor, Competition, LoginCode
)
from app.helpers.email import (
    normalize_email,
    is_admin_email,
    send_login_code_via_email
)
    ...
    if request.method == "POST":
        ...
        if not email:
            ...
            if not error and acct:
                ...
                # Redirect with optional comp context
...
@login_bp.route("/join", methods=["GET", "POST"])
@login_bp.route("/join/", methods=["GET", "POST"])
def public_register():
    """
